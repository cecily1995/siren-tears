...
export default function AnnouncementBar({ items }: { items: AnnouncementItem[] }) {
  ...
  return (
    <div className="fixed inset-x-0 top-0 z-[60] h-7 bg-white border-b border-charcoal/10 overflow-hidden">
      <Link
        href={item.href!}
        className="flex h-full items-center justify-start px-6 md:px-12 text-left text-[9px] sm:text-[10px] tracking-[0.18em] uppercase text-charcoal/80 hover:text-charcoal"
